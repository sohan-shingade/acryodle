...
export class ErrorBoundary extends React.Component<{ children: React.ReactNode }, State> {
  ...
  componentDidCatch(error: Error) {
    if (import.meta.env.DEV) console.error("[acrodle] render crash:", error);
  }
  ...
  render() {
    ...
    return (
      <div style={{ fontFamily: "Helvetica, Arial, sans-serif", maxWidth: 420, margin: "60px auto", padding: 24, textAlign: "center", color: "#1a1a1b" }}>
        <h1 style={{ fontSize: 22, letterSpacing: "0.06em" }}>ACRODLE</h1>
        <p style={{ color: "#787c7e", fontSize: 14 }}>Something went wrong loading the game.</p>
        <button
          onClick={() => {
            try {
              Object.keys(localStorage).filter((k) => k.startsWith("acrodle:")).forEach((k) => localStorage.removeItem(k));
            } catch { /* ignore */ }
            location.reload();
          }}
